...
# function importpy(X::AbstractString; kw...)
#     md=Symbol(replace(X, "." => "!"))
#     importpy(X; kw...)
# end
# function importpy(X::Symbol, md::Symbol=X; preloaded::Bool=false)
#     if !isdefined(Main,md)
#         preloaded && error("Package $X has not been loaded.")
#         S=replace(string(X), "!" => ".")
#         if !isdefined(Main,:PyCall)
#             eval(Main,parse("using PyCall"))
#         end
#         # eval(Main,parse("Main.PyCall.@pyimport $S as $md")) # Will trigger a depwarn.
#         eval(Main, Meta.parse("$md = Main.PyCall.pyimport( $S )"))
#     end
#     getfield(Main, md)::Main.PyCall.PyObject
# end
...
#Xiong Jieyi, 13 May 2014>9 Sep 2014>December 11, 2014>January 6, 2015>Oct 19, 2015>Oct 19, 2015>May 13, 2016>Aug 22, 2016
...
macro c_str(S,P...)
    if isempty(P)
        ...
        @assert(P[1]=="c","Only allow c\"Astr,Bstr\" or c\"Astr#Bstr#\"c.")
        if ismatch(r"\n[\ \t]+$",S)
            D='\n'
            S=rstrip(rstrip(S,' '),'\t')
        end
    ...
    C=map(x->ascii(strip(x)),split(S,D))
    # :($C) #not parse $var
...
if VERSION<v"0.4"
    macro c_mstr(S,P...)
        if isempty(P)
            :(convert(Vector{ASCIIString},map(lstrip,split($S,','))))
        else
            @assert(P[1]=="c","Only allow c\"Astr,Bstr\" or c\"Astr#Bstr#\"c.")
            :(convert(Vector{ASCIIString},map(lstrip,split($(S[1:end-1]),$(S[end])))))
        end
    end
    export @c_mstr
end
...
#{{ @f_str @F_str
#[[ @f_str @F_str ]]
# f"\$1=$1 and $(2)-$3 $(4=.3g) $(5=h)"("aa",100,200,pi,1000) will return string
...
# f"$(N=...)" allows to specify a printf-like format after = ('%' mark should not be included), or human-readable number ($(N=h), transfer-by saynum()), but format a variable one more time is not supported.
# In f"$1-$x" but not in F"$1-$x", the $x will also be replaced.
# See also: @c_str, @cut_str, @k_str
# Xiong Jieyi, Aug 23, 2015 >Nov 4, 2015 >Aug 4, 2016 >17 May 2018

export @f_str, @F_str
macro f_str(T,__insidE_valuE_dO_noT_usE__...)
    ...
    C=map(eachmatch(r"(?<!\\)\$\((\d+)\=([\d\w\.\-\+\#\*]+)\)", T)) do m
        ...
        if f=="h"
            ...
            "__insidE_valuE_dO_noT_usE__[$d]=@sprintf(\"%$f\",__insidE_valuE_dO_noT_usE__[$d])"
        end
    end
    ...
    T=replace(T, '"' => "\\\"")
    esc(Meta.parse("(__insidE_valuE_dO_noT_usE__...)->$I\"$T\""))
end
macro F_str(T,__insidE_valuE_dO_noT_usE__...)
    D=Set{String}() #Just for error promote.
    C=map(eachmatch(r"(?<!\\)\$\((\d+)\=([\d\w\.\-\+\#\*]+)\)", T)) do m
    ...
    Meta.parse("(__insidE_valuE_dO_noT_usE__...)->$I\"$T\"")
end
...
#[[ echo ]]
#Syntax: AbstractString|Any=string(Any) |> echo
#will return `echo AbstractString...`
#used to put string to unix pipe
#See also:
#Xiong Jieyi, Feb 22, 2014
echo(x::AbstractString)=`echo $x`
echo(x)= echo(string(x))
export echo
...
# num2str(X::Tuple; wargs...)=map(x->num2str(x; wargs...), X)
...
#See also: num2str
#Xiong Jieyi, February 8, 2015>Jun 1, 2015
...
#        del=c"var1,...", delmeta=c"mvar1,..."
#        istidystr=true, append=false, rewrite=false)
#
#Dict=jldload(filename[, varnames]; showinfo=isinteractive())
#(Value1,Value2,...)=jldload(filename,varname1,varname2,...; ...)
#
#Dict=jldmeta(filename[, meta_varnames_vec])
#Value1, Value2, ...=jldmeta(filename, meta_varname1, meta_varname2,...)
#Dict, MetaDict = jldload2(filename) #No meta-tip show in screen.
...
function jldload(filename::AbstractString,varnames::Vector{Tv}=ASCIIString[]; showinfo::Bool=isinteractive()) where {Tv<:ByteString}
    @pkgfun(JLD, jldopen=>JLD_jldopen)
...
function jldlook(filename::ByteString; showinfo::Bool=isinteractive())
    # isdefined(:HDF5)||require("HDF5")
    # isdefined(:JLD)||require("JLD")
    @pkgfun(JLD, jldopen=>JLD_jldopen)
    @pkgfun(HDF5, exists=>HDF5_exists)
...
function jldmeta(filename::AbstractString, varnames::Vector{Ts}=ByteString[]) where {Ts<:ByteString}
    @pkgfun(JLD, jldopen=>JLD_jldopen)
    @pkgfun(HDF5, exists=>HDF5_exists)
...
function jldreadme(filename)
    jldmeta(filename,"readme")
end
function jldload2(filename::AbstractString, args...)
    O1=jldload(filename, showinfo=false)
    O2=jldmeta(filename, args...)
    if length(args)>1
        tuple(O1, O2...)
    else
        (O1, O2)
...
function jldload2(arg1::Tuple, args...)
    O1=jldload(arg1..., showinfo=false)
    O2=jldmeta(arg1[1], args...)
    if length(args)>1
        tuple(O1, O2...)
    else
        (O1, O2)
...
#
#Dict=jmeta(filename[, meta_varnames_vec])
#Value1, Value2, ...=jmeta(filename, meta_varname1, meta_varname2,...)
#Dict, MetaDict = jload2(filename) #No meta-tip show in screen.
...
#readme_str = jreadme(filename)
#
#Saving and load data in JLD2 format. Filename will be added .jld2 automatically.
#When allow_any = false, saving Array{Any} data is not allowed (It's slow to save a Array{Any}, and in most cases it is not necessary.).
#When istidystr is true, tidystr will be used to convert the input data.
#When append is true, new variable or meta could be added in the old file.
#When rewrite is true, new variable with the same name of old variable will be rewritted. Otherwise a error is occur. rewrite=true imply append=true. (This function is currently unvaliable.)
#When del or delmeta is given, the given variable will be removed before saving.
#See also: jldsave jldload jldlook jldreadme jldmeta, ds, tidy, tidytype, matload, matsave, matlook
#Xiong Jieyi, 3 Sep 2018

export jsave, jload, jlook, jmeta, jload2, jreadme

#__trslash(S)=replace(S, r"/" => "\\%") #Already defined in jld... functions.
#__reslash(S)=replace(S, r"\\%" => "/")
function jsave(filename::AbstractString, data::Dict{T1,}=Dict{String,Any}(); istidystr::Bool=true, readme::AbstractString="", meta::Dict{T2,}=Dict{String,Any}(),append::Bool=false,rewrite::Bool=false,del::Vector{T3}=String[],delmeta::Vector{T4}=String[],allow_any::Bool=false) where {T1<:String,T2<:String,T3<:String,T4<:String}
    ...
    JLD_o_delete(t, x)=error("Delete variable $x is not supported by JLD2 so far.")
    if !allow_any
        is_any=isa.(collect(values(data)), Array{Any})
        if any(is_any)
            error(f"Variable $1 is Array{Any}. Set allow_any=true."(collect(keys(data))[findfirst(is_any)]))
...
function jload(filename::AbstractString, varname1::String, varname2::String, varnames::String...)
    T=jload(filename,[varname1, varname2, varnames...])
    map(x->T[x],tuple(varname1, varname2, varnames...))
end
...
function jmeta(filename::AbstractString, varnames::Vector{T}=String[]) where {T<:String}
    @pkgfun(JLD2, jldopen=>JLD_jldopen, keys=>JLD_keys, haskey=>HDF5_exists)
...
function jload2(filename::AbstractString, args...)
    O1=jload(filename, showinfo=false)
    O2=jmeta(filename, args...)
    if length(args)>1
        tuple(O1, O2...)
    else
        (O1, O2)
...
function jload2(arg1::Tuple, args...)
    O1=jload(arg1..., showinfo=false)
    O2=jmeta(arg1[1], args...)
    if length(args)>1
        tuple(O1, O2...)
    else
        (O1, O2)
...
macro sf_str(T,__insidE_valuE_dO_noT_usE__...)
    T=replace(T, r"\\(?!\$(\d|\(\d+\)))" => "\\\\")
    T=replace(T, '"' => "\\\"")
...
#Convert the values of a Dict to tuple.
#See also: fd
#Xiong Jieyi, 7 Sep 2014 > 22 Nov 2018

dict2tuple(D::Dict, Fs)=tuple([D[f] for f in Fs]...)
dict2tuple(D::Dict)=tuple(values(D)...)
export dict2tuple
#}}

#{{ promote_array promote_tuple promote_group promote_anyrow
...
#See also: promote_tuple, promote_group, promote, promote_rule, promote_type
#Xiong Jieyi, 9 Sep 2014

#<v0.6# function promote_array{AT,BT}(A::AbstractVector{AT},B::AbstractVector{BT})
...
#<v0.6# function promote_array{AT,BT}(A::AbstractMatrix{AT},B::AbstractMatrix{BT})
...
#Promote each type of element in the group. Function will choose promote_array or promote_tuple to run depend on the types.
...
#  Experimentially. @pyf_str will create a lambda function. Only support one line. Require PyCall being pre-loaded.
#
#See also: @pyhelp
#Xiong Jieyi, 16 Sep 2014>December 8, 2014>4 Mar 2019>26 Jun 2019
...
#See also: segpileup
#Xiong Jieyi, October 19, 2014

function drawer(N::Real,G::Real)
    D,R=divrem(N,G)
    O=fill(D,G)
    O[1:R]+=1
    O
...
#[[ filefun filefunwith fileloop fileiofun ]]
# (Array1,Array2,...)=filefun(function, filename|Cmd|IO; input_rowno|no=false, multi_output=false, skipline=0, ignore::Char='\0', ischomp=true, each=0, maxrow=0)
# ((Array1,Array2,...), row_NO)=filefunwith(function, filename|Cmd|IO; input_rowno|no=false, multi_output=false, rowexp=false, skipline=0, ignore::Char='\0', ischomp=true, each=0, maxrow=0)
...
function filefun(fun::Function,fid::IO;multi_output::Bool=false,skipline::Integer=0,ignore::Char='\0', maxrow::Integer=0, wargs...)
    for i=1:skipline
        readline(fid)
    end
    rlt=rowpile()
    ...
    if rownum(rlt)==0
        @warn("No output collected, return nothing instead.")
        return nothing
    else
...
function filefunwith(fun::Function,fid::IO;multi_output::Bool=false,skipline::Integer=0,ignore::Char='\0', maxrow::Integer=0, wargs...)
    for i=1:skipline
        readline(fid)
    end
    rlt=rowpile()
...
# filename_vec|[""] = matchfile("filename_with_?|*"; onlyone=false, hasone=onlyone)
...
#{{ capstdout
#[[ capstdout ]]
# ...=capstdout(fun[, filename|Cmd|pipe]; isshow=false)
#Capture function stand output to file. If filename is omitted, function just hide the output.
#e.g.
#  capstdout("to_file.txt") do
#      #do something here...
#  end
#See also: KK, writeall, runstr, successstr.
#Xiong Jieyi, Jun 17, 2015 >Jan 15, 2016

export capstdout
function capstdout(fun::Function, file=nothing; isshow::Bool=false)
    oldout=stdout
    fout=nothing
    ...
    if isshow || file!=nothing
        txt=readavailable(out)
        if isshow
            println(join(convert(Vector{Char}, readavailable(out))))
        end
        if file!=nothing
            write(file,txt)
            t=saynum(length(txt))
            @info("Output (length $t) was redirected to $file.")
        end
    end
    fout
end
...
function getobj(S::AbstractString)
    ...
    _getobj(S::QuoteNode)=S.value
    _getobj(S)=S
    function _getobj(S::Expr)
        md=_getobj(S.args[1])
        ...
        elseif S.head==:ref
            ...
            if isa(md, Union{AbstractArray, Tuple})
                ...
                return get(md, ss[1], undef)
            end
        end
    end
    _getobj(Meta.parse(S))
end
#}}
...
#Or obj=ParseStr(txt; start=1); next(obj, commands...)
#
...
#See also: readall
#Xiong Jieyi, Sep 22, 2015 >Oct 7, 2015>Nov 17, 2015>Feb 5, 2016>Apr 15, 2016>May 12, 2016>May 20, 2016>Jan 18, 2017>13 Jun 2017>9 Jan 2019

export parsestr
let
    global parsestr
    function parsestr(txt,cmds...;start_ref::Vector{Int}=[1],start::Int=start_ref[1],prev_ref::Vector{Int}=[1],lastMatchRange_ref::Vector{Trg}=[0:-1]) where {Trg<:Range}
...
#{{ parsestr2
#[[ parsestr2 ]]
# NOTE: parsestr2 is old version of parsestr. As the code of parsestr is more neat and mainteinable, parsestr2 will be absolated in the feature.
#
# Any[value1, value2, ...]=parsestr(txt, commands...; start=1|start_ref=[start])
#Or obj=ParseStr(txt; start=1); next(obj, commands...)
#
# Find txt from the start position. The commands is found by order. Supported commands are: AbstractString, Regex, Int. Int means moving cursor or fetch next/pervious worlds. 
# (command, [function]) means fetch the matched word (or first token if have) and apply the function. If command is AbstractString, fetch all string before first match (=(:getbefore, "...")). When Regex has token, only return the first token, but start position is desided by the whole match.
# (:try, command*) Try to find pattern. Ignore error if failed.
# (:try, command*, default) Try to fetch data. If failed, report default instead. (:try, (command,)) is equal to (:try, command, nothing).
# (:check, command) Check if command is matched, return true or false. Change start point if matched.
# (:before|:back, commmand*) Run command only in the region between last match and the match before last match. :back will also move start point back but :before will not.
# (:before, function) Input before-substring to function and throw an error if function not return true. It can be used as an assert. e.g. (:before, x->length(x)<10).
# (:no, command) Throw a error if matched. Usually used along with (:before, (:no, "impossible_string"))
# (:getbefore|:getuntil, command[, function]) Get the string between the start point and this match. :getuntil will ignore token in command, and :getbefore only allow token at the end of command.
# start_ref can be used to both input and return the next start position before and after processed all commands.
#Function always return a Any-vector.
#See also: readall
#Xiong Jieyi, Sep 22, 2015 >Oct 7, 2015>Nov 17, 2015>Feb 5, 2016

export parsestr2
function parsestr2(txt,P...;start_ref::Vector=[1],start::Int=start_ref[1])
    ...
    for cP in P
        ...
        while flag
            ...
            if isa(cP,Tuple)
                if cP[1]==:try
                    istry=true
                    if length(cP)>2
                        @assert(length(cP)==3,"Input should be in (:try, (Pattern, [Function]), Default) format.")
                        default=cP[3]
                    end
                    cP=cP[2]
                    flag=true
                elseif cP[1]==:check
                    @assert(length(cP)==2,"Invalid format (:check, cmd)")
                    ischeck=true
                    cP=cP[2]
                elseif cP[1]==:before
                    @assert(length(cP)==2,"Invalid format (:before, cmd)")
                    cP=cP[2]
                    if isa(cP,Function)
                        @assert(cP(SubString(txt,previous,start-1)))
                        continue_for=true
                    else
                        isbefore=true
                        flag=true
                ...
                elseif cP[1]==:back
                    @assert(length(cP)==2,"Invalid format (:back, cmd)")
                    cP=cP[2]
                    isback=true
                    flag=true
                elseif cP[1]==:no
                    @assert(length(cP)==2,"Invalid format (:no, cmd)")
                    cP=cP[2]
                    isno=true
                elseif cP[1]==:getbefore
                    if isa(cP[2],AbstractString)
                        cP=cP[2:end]
                        isvalue=true
                    else
                        if length(cP)==3
                            ...
                            @assert(length(cP)==2,"Invalid format (:getbefore, cmd[, default])")
                        end
                        cP=cP[2]
                        if !isa(cP,Tuple)
                            cP=(cP,)
                ...
                elseif cP[1]==:getuntil
                    if length(cP)==3
                        getbeforefun=cP[3]
                    else
                        @assert(length(cP)==2,"Invalid format (:getuntil, cmd[, default])")
                    end
                    cP=cP[2]
                    isgetuntil=true
                    flag=true
        ...
        if isbefore
            isfound,ignore,val=_parsestr_core(SubString(txt,1,start-1),cP,previous)
        elseif isback
            isfound,start,val=_parsestr_core(SubString(txt,1,start-1),cP,previous)
        else
            previous=start
            isfound,start,val=_parsestr_core(txt,cP,start)
        end
        if isno
            @assert(!isfound,"Found $cP")
        elseif ischeck
            push!(O,isfound)
        else
            if isfound
                if isgetbefore
                    vallen=length(val)
                    @assert(!isa(cP[1],Regex) || txt[start-vallen:start-1]==val,
                            "In (:getbefore, r\"..(..)\") mode, token only allowed at the end of Regex.")
                    push!(O,getbeforefun(txt[previous:start-length(val)-1]))
                elseif isgetuntil
                    push!(O,getbeforefun(txt[previous:start-1]))
                elseif isvalue
                    push!(O,val)
                end
            else
                if !istry
                    error("Cannot found $val")
                elseif isvalue||isgetbefore||isgetuntil
                    push!(O,default)
                end
            end
        end
...
function _parsestr_core(txt::AbstractString,P::Regex,N::Int)
    M=match(P,txt,N)
    if M==nothing
        return (false,N,string(P))
    end
    pN=M.offset+length(M.match)
    (true,pN,nothing)
end
function _parsestr_core(txt::AbstractString,P::Tuple{Regex},N::Int)
    P=P[1]
    M=match(P,txt,N)
    if M==nothing
        return (false,N,string(P))
    ...
    if isempty(M.captures)
        ...
        (true,pN,M.captures[1])
    end
end
function _parsestr_core(txt::AbstractString,P::AbstractString,N::Int)
    R=search(txt,P,N)
    if isempty(R)
        return (false,N,string(P))
    end
    (true,R[end]+1,nothing)
end
function _parsestr_core(txt::AbstractString,P::Tuple{AbstractString},N::Int)
    P=P[1]
    R=search(txt,P,N)
    if isempty(R)
        return (false,N,string(P))
    end
    (true,R[end]+1,txt[N:R[1]-1])
end
function _parsestr_core(txt::AbstractString,P::Int,N::Int)
    if N+P-1>length(txt)
        return (false,N,"cursor-move-($P)")
    end
    (true,N+P,nothing)
end
function _parsestr_core(txt::AbstractString,P::Tuple{Int},N::Int)
    P=P[1]
    if P<0
        if N+P<1
            return (false,N,"cursor-move-($P)")
        ...
        (true,N+P,txt[N:N+P-1])
    end
end
function _parsestr_core(txt::AbstractString,P::Tuple{Any,Function},N::Int)
    stat,pN,val=_parsestr_core(txt,(P[1],),N)
    if !stat
        return (false,pN,val)
    end
    (true,pN,P[2](val))
